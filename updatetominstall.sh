...
for n in $(cat serverlist.txt)

do

  ssh -i "/Users/apple/Downloads/vijayshellscript/tominstall.pem" ec2-user@$n 'p=$(ps -ef | grep tomcat | grep Djava.util.logging.config.file | grep -v grep | awk "{print \$2}"); 
